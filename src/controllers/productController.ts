...
export const getLatestProduct = TryCatch(
  async (
    ...
    if (myCache.has("latest-product")) {
      latestProduct = JSON.parse(myCache.get("latest-product") as string);
    } else {
...
export const getAllCategory = TryCatch(
  async (
    ...
    if (myCache.has("all-categories")) {
      allCategories = JSON.parse(myCache.get("all-categories") as string);
    } else {
...
export const getSingleProduct = TryCatch(
  async (
    req: Request<{ id: any }, {}, NewProductRequestBody>,
    res: Response,
    next: NextFunction
  ) => {
    const { id } = req.params;
    ...
    if (myCache.has(`product-${id}`)) {
      ...
      if (!id) return next(new ErrorHandler("Please provide a Id ", 401));
...
export const getAllFilterProduct = TryCatch(
  async (
